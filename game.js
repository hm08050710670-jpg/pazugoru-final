...
async function enemyShot(epoch){
  ...
  const menuMapButton=$('menuMapButton');
  ...
  async function loadBattle(id){
    ...
    try{
      ...
      try{if(spriteImage.decode)await spriteImage.decode();else if(!spriteImage.complete)await new Promise((res,rej)=>{spriteImage.onload=res;spriteImage.onerror=rej;});}catch(e){}
      if(seq!==loadSerial)return;
